...
export default function InterpolacionLagrange() {
  ...
  return (
    <Paper sx={s.p}>
      <Typography variant="h5" gutterBottom sx={s.t}>Interpolación de Lagrange</Typography>
      <Stack spacing={2.5}>
        <TextField label="Valores de X (separados por coma)" value={xValues} onChange={(e) => setXValues(e.target.value)} placeholder="Ej: 1, 2, 3" sx={s.i} />
        <TextField label="Valores de Y (separados por coma)" value={yValues} onChange={(e) => setYValues(e.target.value)} placeholder="Ej: 2, 3, 5" sx={s.i} />
      ...
      {resultado && (
        <Stack sx={{ mt: 4, animation: 'fadeInUp 0.5s ease-out' }}>
          ...
          <Typography sx={{ mt: 2, mb: 1, color: 'var(--calc-text-secondary)' }}>Pasos del cálculo:</Typography>
          <Table size="small" sx={s.tb}>
            <TableHead><TableRow><TableCell>i</TableCell><TableCell>xᵢ</TableCell><TableCell>yᵢ</TableCell><TableCell>Lᵢ(x)</TableCell><TableCell>Término</TableCell></TableRow></TableHead>
            <TableBody>
