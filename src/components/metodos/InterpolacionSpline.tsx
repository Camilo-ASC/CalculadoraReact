...
export default function InterpolacionSpline() {
  ...
  const calcular = () => {
    try {
      ...
      if (x.length !== y.length) throw new Error("Los vectores X e Y deben tener la misma longitud.");
  ...
  return (
    <Paper sx={s.p}>
      <Typography variant="h5" gutterBottom sx={s.t}>Interpolación Spline Cúbica</Typography>
      <Stack spacing={2.5}>
        <TextField label="Valores de X (separados por coma)" value={xValues} onChange={(e) => setXValues(e.target.value)} placeholder="Ej: 1, 2, 3, 4" sx={s.i} />
        <TextField label="Valores de Y (separados por coma)" value={yValues} onChange={(e) => setYValues(e.target.value)} placeholder="Ej: 1, 4, 9, 16" sx={s.i} />
      ...
      {resultado && (
        <Stack sx={{ mt: 4, animation: 'fadeInUp 0.5s ease-out' }}>
          ...
          <Typography sx={{ mt: 2, mb: 1, color: 'var(--calc-text-secondary)' }}>Coeficientes de cada tramo:</Typography>
          <Table size="small" sx={s.tb}>
            <TableHead><TableRow><TableCell>Intervalo</TableCell><TableCell>a</TableCell><TableCell>b</TableCell><TableCell>c</TableCell><TableCell>d</TableCell></TableRow></TableHead>
            <TableBody>
